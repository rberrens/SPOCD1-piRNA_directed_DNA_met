...
L1MdA_met_tidy <- met_L1mdA %>% distinct() %>% na.omit() %>%
  select(Chr, Start, End, Strand, ID, Description, Mean.Miwi2_KO, Mean.Spocd1_KO) %>%
  filter(Description!="No description") %>% 
  mutate(Miwi2_WT = Mean.Miwi2_KO-Mean.WT, Spocd1_WT = Mean.Spocd1_KO-Mean.WT)

L1MdA_met_tidy$Description <- as.integer(L1MdA_met_tidy$Description)
  
L1MdT_met_tidy <- met_L1mdT %>% distinct() %>% na.omit() %>%
  select(Chr, Start, End, Strand, ID, Description, Mean.Miwi2_KO, Mean.Spocd1_KO, Mean.C19_KO, Mean.WT) %>%
  filter(Description!="No description") %>%
  mutate(Miwi2_WT = Mean.Miwi2_KO-Mean.WT, Spocd1_WT = Mean.Spocd1_KO-Mean.WT)
